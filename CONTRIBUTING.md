...
PR 통과 조건. 새 모듈은 같은 max-line-length 따름.
...
- **에이전트 진입점**: [`CLAUDE.md`](CLAUDE.md) (Claude Code). 다른
  에이전트 (Codex, Cursor, Copilot 등) 공용 진입점 (`AGENTS.md`) 도입은
  로드맵.
...
- [ ] `pytest -q` 통과
- [ ] `pycodestyle --max-line-length=120 hometax_client/` 통과
